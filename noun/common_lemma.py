...
with open('lemma_metaphor_id.json', mode = 'r') as f:
    lemma_metaphor_id = json.load(f)

with open('lemma_nonmetaphor_id.json', mode = 'r') as f:
    lemma_nonmetaphor_id = json.load(f)

with open('lemma_count.json', mode = 'r') as f:
...
common_lemma_counts = sorted(common_lemma_counts.items(), key = lambda x:x[1], reverse=True)

with open('common_lemma.json', mode = 'wt', encoding = 'utf-8') as f:
   json.dump(common_lemma_counts, f, ensure_ascii = False, indent = 2)